...
export class ClientOtherDeailsComponent implements OnInit {
  ...
  constructor(
    ...
    private createCustomerService: ClientService,
  ...
  otherDetailsTabData(customerId) {
    this.createCustomerService.getotherdetailstab(customerId).subscribe({
      next: (resp: any) => {
  ...
  SubmitForm() {
    ...
    if (this.form.valid) {
      ...
      this.createCustomerService
        .UpdateOtherDetails(formData, this.cid)
        .subscribe((p) => {
          if (p) {
      ...
      this.snackBarService.showSnackbarTopPosition("Invalid form", "error")
    }
  }
  ...
  // @rahul ask- no api avail
  getTaxPref() {
    this.createCustomerService.getTaxPreference().subscribe({
      next: (list: any) => {
